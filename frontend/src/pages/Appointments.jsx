...
const Appointments = () => {
  ...
  const fetchDocInfo = () => {
    if (!doctors || !Array.isArray(doctors)) return; // Safeguard against undefined
  ...
  const getAvailableSlots= async()=>{
    ...
    for(let i=0;i<7;i++)
    {
      ...
      if (today.getDate()=== currentDate.getDate()){
        currentDate.setHours(currentDate.getMinutes>30? currentDate.getHours()+1:currentDate.getHours())
        ...
        <div className='flex flex-col sm:flex-row gap-4'>
          ...
          <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
            ...
            <p className='text-gray-500 font-medium mt-4'>Appointment fee:
              <span className='text-gray-600'> {docInfo.fees}{currencySymbol}</span>
            </p>
          </div>    
          </div>
          {/* Booking Slots */}
